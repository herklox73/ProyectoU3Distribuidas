...
function actualizarListaChats() {
    ...
    for (let numero in todasLasConversaciones) {
        ...
        if (ultimoMensaje.tipo === 'enviado') {
            previewPrefix = '<span class="text-muted me-1">✓✓</span>';
        }
...
function mostrarMensajes(numero) {
    ...
    conversacion.mensajes.forEach(msg => {
        ...
        const msgIdAttr = msg.id ? `data-msg-id="${msg.id}"` : '';
        const statusId  = msg.id && esEnviado ? `id="status-icon-${msg.id}"` : '';

        const mensajeHTML = `
            <div class="d-flex flex-column ${esEnviado ? 'align-items-end' : ''} mb-3" ${msgIdAttr}>
                <div class="rounded p-2 ${esAudio ? 'bg-primary-soft' : (esEnviado ? 'bg-light' : 'bg-primary-soft')}"
                     style="min-width: ${esMedia || esDocumento || esAudio || esVideo ? 'auto' : '20%'};
                            max-width: ${esMedia ? (esSticker ? '220px' : '320px') : (esDocumento ? '400px' : (esAudio ? '350px' : (esVideo ? '420px' : '60%')))};
                            ${estilosContainer}
                            display: inline-block;">
                    ${contenido}
                </div>
                <small class="${esEnviado ? 'text-muted me-2' : 'text-primary fw-bold ms-2'}" style="margin-top: 4px;">
                    ${msg.hora}
                    ${esEnviado ? `<span ${statusId}>${obtenerIconoEstado(msg.status)}</span>` : ''}
                </small>
            </div>
        `;
...
// ===============================================
// POLLING DE ESTADOS EN TIEMPO REAL (ticks ✓ ✓✓ 👁)
// ===============================================
let statusPollingActivo = false;

function iniciarPollingEstados() {
    if (statusPollingActivo) return;
    statusPollingActivo = true;

    function pollEstados() {
        if (!statusPollingActivo) return;

        // Recopilar IDs de mensajes enviados visibles en el chat activo
        const chatMessages = document.querySelector('.chat-messages');
        if (!chatMessages) {
            setTimeout(pollEstados, 5000);
            return;
        }

        const msgDivs = chatMessages.querySelectorAll('[data-msg-id]');
        const ids = [];
        msgDivs.forEach(div => {
            const id = parseInt(div.getAttribute('data-msg-id'));
            // Solo mensajes que tengan status-icon (son outbound)
            if (id > 0 && div.querySelector('[id^="status-icon-"]')) {
                ids.push(id);
            }
        });
        ...
        .then(data => {
            if (data.updates && data.updates.length > 0) {
                ...
                data.updates.forEach(upd => {
                    const span = document.getElementById(`status-icon-${upd.id}`);
                    if (!span) return;

                    const iconoNuevo = obtenerIconoEstado(upd.status);
                    const iconoActual = span.innerHTML;

                    if (iconoActual !== iconoNuevo) {
                        span.innerHTML = iconoNuevo;
                        huboCAmbio = true;

                        // Actualizar también en el objeto de conversación local
                        if (chatActivo.numero && todasLasConversaciones[chatActivo.numero]) {
                            const msgs = todasLasConversaciones[chatActivo.numero].mensajes;
                            const msgLocal = msgs.find(m => m.id == upd.id);
                            if (msgLocal) msgLocal.status = upd.status;
                        }
                    }
                });
            }

            if (statusPollingActivo) {
                setTimeout(pollEstados, 5000);
            }
        })
        .catch(() => {
            if (statusPollingActivo) {
                setTimeout(pollEstados, 8000);
            }
        });
    }
...
function detenerPollingEstados() {
    statusPollingActivo = false;
}
// ===============================================
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Iniciar polling de estados de mensajes (ticks ✓ ✓✓ azul)
